;;; repeatable.el --- Repeatable prefix commands with which-key -*- lexical-binding: t; -*-
...
;; repeatable provides a lightweight system for making prefix key commands
...
;;
;; The main entry point is the `repeatable-wrap' macro, which wraps any
...
(defvar repeatable--saved-settings nil
  "Plist of saved which-key settings, or nil when no repeatable loop is active.
A non-nil value also serves as the active flag.")

(defcustom repeatable-help-backends
  '((?\C-h "C-h" "which-key" repeatable--prefix-help))
  "Alist of help backends for `repeatable--versatile-C-h'.
...
(defcustom repeatable-dismiss-key ?q
...
(defvar repeatable--prefix-help-pending nil
  "Non-nil when waiting for the real command after prefix help.")

(defun repeatable--restore-after-prefix-help ()
...
command executes."
  (cond
   (repeatable--prefix-help-pending
    (setq repeatable--prefix-help-pending nil))
   ((member this-command which-key--paging-functions)
    nil)
   (t
    (remove-hook 'post-command-hook #'repeatable--restore-after-prefix-help)
    (repeatable--kill-which-key))))

(defun repeatable--which-key-dispatch-or-switch ()
  ...
  (if (not (which-key--popup-showing-p))
    ...
    (let* ((prefix-keys (which-key--current-key-string))
           ...
           (switch-entries
            (seq-filter
             (lambda (e)
               (not (eq (nth 3 e) #'repeatable--prefix-help)))
             repeatable-help-backends))
           (dismiss-label (key-description (vector repeatable-dismiss-key)))
           (switch-prompt
            (when switch-entries
              (concat " "
                      (mapconcat
           ...
           (backend (assq ev repeatable-help-backends)))
      (setq this-command 'which-key-C-h-dispatch)
      (cond
       ((eq ev repeatable-dismiss-key)
        (remove-hook 'post-command-hook #'repeatable--restore-after-prefix-help)
        (repeatable--kill-which-key repeatable--help-prefix))
       ((and backend
             (not (eq (nth 3 backend) #'repeatable--prefix-help)))
        (repeatable--switch-from-which-key ev))
       (t
...
(defun repeatable--prefix-help (_keymap prefix)
...
idle timer as the user navigates sub-prefixes."
  ...
  (setq prefix-help-command 'repeatable--which-key-dispatch-or-switch)
  (setq repeatable--prefix-help-pending t)
  (add-hook 'post-command-hook #'repeatable--restore-after-prefix-help))

(defun repeatable--kill-which-key (&optional replay-keys)
...
command loop so Emacs continues from those keys."
  ...
  (when repeatable--saved-settings
    (setq which-key-idle-delay (plist-get repeatable--saved-settings :idle-delay)
          which-key-idle-secondary-delay (plist-get repeatable--saved-settings :idle-secondary-delay)
          which-key-persistent-popup (plist-get repeatable--saved-settings :persistent-popup)
          echo-keystrokes (plist-get repeatable--saved-settings :echo-keystrokes)
...
(defun repeatable--call-backend (backend km prefix)
  "Call BACKEND with KM and PREFIX, handling switch and dismiss.
If the backend throws to `repeatable-switch' with another
...
the prefix is replayed so the user can continue typing."
  (let ((next-key (catch 'repeatable-switch
                    (condition-case nil
    ...
    (cond
     ((eq next-key :dismiss)
      (which-key-reload-key-sequence prefix))
     (next-key
      (let ((target (assq next-key repeatable-help-backends)))
        (when target
          (repeatable--call-backend target km prefix)))))))

(defun repeatable--switch-from-which-key (target-key)
  "Switch from which-key help to the backend at TARGET-KEY."
  (remove-hook 'post-command-hook #'repeatable--restore-after-prefix-help)
  (repeatable--kill-which-key)
  (let ((backend (assq target-key repeatable-help-backends)))
    (when backend
      (repeatable--call-backend
       backend
       repeatable--help-keymap
       repeatable--help-prefix))))

(defun repeatable--switch-from-minibuffer (target-key)
  "Switch from a minibuffer-based backend to the backend at TARGET-KEY.
Throws to the `repeatable-switch' catch tag, unwinding the
minibuffer and any intermediate calls."
  (interactive)
  (throw 'repeatable-switch target-key))

(defun repeatable-setup-minibuffer-switches (current-handler)
...
\\`C-S-h') to avoid the built-in help system intercepting them."
  (dolist (entry repeatable-help-backends)
    (unless (eq (nth 3 entry) current-handler)
      (let* ((key (car entry))
        ...
        (local-set-key (vector bind-key)
          (lambda () (interactive)
            (repeatable--switch-from-minibuffer key)))))))

(defun repeatable--versatile-C-h ()
  "Dispatch to a help backend after \\`C-h' press in a prefix sequence.
Available backends are configured via `repeatable-help-backends'."
  (interactive)
  (let* ((keys (this-command-keys-vector))
         ...
         (prompt (concat
                  ...
                  ":"))
         (key (read-key prompt))
         (backend (assq key repeatable-help-backends)))
    (setq repeatable--help-keymap km
          repeatable--help-prefix prefix)
    (if backend
        (repeatable--call-backend backend km prefix)
      (message "Invalid key"))))

(defun repeatable--process-undefined (&optional ksv)
  ...
  (let* ((ksv (or ksv (this-single-command-keys)))
    ...
    (cond
     ...
     (t (repeatable--kill-which-key)))))

(defun repeatable--popup-live-p ()
  "Return the which-key popup buffer when it is alive, else nil."
  (get-buffer which-key-buffer-name))

(defun repeatable--wrapped-command-p (binding)
  "Return non-nil when BINDING is a `repeatable-wrap' generated command.
Such commands re-enter the repeatable loop themselves, so the
which-key popup must be left in place when they run."
  (and (symbolp binding)
       (string-prefix-p "repeatable-wrap-" (symbol-name binding))))

(defun repeatable--read-key-sequence ()
         ...
         (t
          (let* ((last-key-vector (vector (aref ksv (1- (length ksv)))))
            ...
            (cond
             ...
             ((string= last-key "C-u")
              (repeatable--process-undefined ksv))
              ...
              (if (keymapp last-key-binding)
                  (if (repeatable--popup-live-p)
                      (progn
                        ...
                        (setq reading t))
                    (repeatable--kill-which-key last-key-vector))
                (repeatable--kill-which-key)
             ...
             ((string= last-key "C-S-x")
              (repeatable--kill-which-key [24]))
             (t
              (message "No binding in local or global maps %s" key)
              (repeatable--kill-which-key))))))))))

;;;###autoload
(defmacro repeatable-wrap (function)
...
Usage:
  (define-key my-map \"h\" (repeatable-wrap windmove-left))
  (define-key my-map \"l\" (repeatable-wrap windmove-right))"
  (unless (symbolp function)
    (error "`repeatable-wrap' requires a symbol, got %S"
           function))
  `(defun ,(intern (format "repeatable-wrap-%s" function)) ()
     (interactive)
     (let* ((keys (this-command-keys-vector))
...
prefix keymap active after a command executes."
  ...
  (if repeatable-mode
      (progn
        (advice-add 'which-key-C-h-dispatch :before #'repeatable--which-key-settings)
        (advice-add #'keyboard-quit :before #'repeatable--kill-which-key)
        (advice-add #'undefined :override #'repeatable--process-undefined))
    (advice-remove 'which-key-C-h-dispatch #'repeatable--which-key-settings)
    (advice-remove #'keyboard-quit #'repeatable--kill-which-key)
    (advice-remove #'undefined #'repeatable--process-undefined)))
